<template>
    <div class="clearfix increasable_div" v-show="isShow">
        <div style="position: relative; float: left; margin-right: 10px;">
            <div class="cloud_storage">SSD云盘</div>
        </div>
        <div class="fl">
            <div class="clearfix manual_input_box">
                <inputLimitStep class="manual_input" v-model.number="valObj.dataDiskSize" v-bind:max="1024" v-bind:min="50" v-bind:step="10"></inputLimitStep><span class="broadband_unit">GB</span>
            </div>
        </div>
