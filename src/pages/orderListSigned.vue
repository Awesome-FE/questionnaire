<template>
    <div class="w clearfix">
        <div class="myOrder_tab_head">
            <router-link class="all" style="letter-spacing: 3px;" :to="{path: '/orderList'}">全部订单</router-link>
            <router-link class="waiting" :to="{path: '/orderListWait'}">待支付订单</router-link>
            <a href="javascript:;" class="selected signed">已支付订单</a>
        </div>
        <div class="myOrder_tab_body">
            <div class="already_sign" v-cloak>
                <table v-if="screenWidth > 767" v-for="order in orders">
                    ...
                    <tr>
                        ...
                        <th colspan="4" style="text-align: right; padding-right: 30px;"><span>{{order.createTime | time}}</span>&nbsp;&nbsp;&nbsp; :to="{path: '/independentOrderDetail', query: {orderId: order.orderId}} "><span>订单号：</span><span class="color_six">{{order.orderSn}}</span></router-link></th>
                    </tr>
                    </thead>
                    <thead v-if="order.orderType == 4">
                    <tr>
                        <th colspan="3" class="text_left" style="padding-left: 30px;">{{order.orderName}}</th>
                        <th colspan="4" style="text-align: right; padding-right: 30px;"><span>{{order.createTime | time}}</span>&nbsp;&nbsp;&nbsp;<router-link target="_blank" :to="{path: '/innovationOrderDetail', query: {orderId: order.orderId}} "><span>订单号：</span><span class="color_six">{{order.orderSn}}</span></router-link></th>
                    </tr>
                    </thead>
                    ...
                    <tr v-for="item in order.goodsItems">
                        ...
                        <td class="check_status theme_color">
                            ...
                            <div><router-link class="order_detail" target="_blank" :to="{path: '/independentOrderDetail', query: {orderId: order.orderId}} ">订单详情</router-link></div>
                        </td>
                        <td class="compact">
                    ...
                    <tr v-for="(item, index) in order.goodsItems">
                        ...
                        <td v-if="index == 0" v-bind:rowspan="order.goodsItems.length" class="total_price red">{{order.orderTotalPrice}}元</td>
                        <td v-if="index == 0" v-bind:rowspan="order.goodsItems.length" class="check_status theme_color">
                            {{order.orderStatus | order_status}}
                            <div><router-link class="order_detail" target="_blank" :to="{path: '/innovationOrderDetail', query: {orderId: order.orderId}} ">订单详情</router-link></div>
                        </td>
                        <td v-if="index == 0" v-bind:rowspan="order.goodsItems.length" class="compact">
                            <a href="javascript:;" class="theme_color export_compact" @click="exportWait">导出合同</a>
                        </td>
                    </tr>
                    </tbody>
                    ...
                    <tr v-for="(item, index) in order.goodsItems">
                        <td>地域</td>
                        <td>{{item.terrain}}</td>
                        <td v-if="index == 0" rowspan="8" class="total_price red" style="min-width: 90px;">{{order.orderTotalPrice}}元</td>
                        <td v-if="index == 0" rowspan="8" class="check_status theme_color">
                            {{order.orderStatus | order_status}}
                            <div><router-link class="order_detail" target="_blank" :to="{path: '/mixedOrderDetail', query: {orderId: order.orderId}} ">订单详情</router-link></div>
                        </td>
                        <td v-if="index == 0" rowspan="8" class="compact">
