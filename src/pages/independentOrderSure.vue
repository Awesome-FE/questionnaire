<template>
    <div class="w">
        ...
        <div class="orderSure_total_price" style="font-size: 14px; color: #999;">总价：<span class="red" style="font-size: 30px;">{{independent.productUnitPrice * independent.productQuantity}}</span><span class="red" style="font-size: 14px">元</span></div>
...
export default {
    ...
    submitOrder() {
      ...
      axios({
        ...
        .then(function(response) {
          ...
          vueObj.$router.push({
            path: "/payWayThird",
            query: { orderId: orderId, orderType: "3" }
          });
        })
  ...
  mounted() {
    ...
    var independentCart = store.get("independentCart");
    if (independentCart == null || typeof independentCart == "undefined") {
      errorMessage = "没有独立办公空间产品提交，请重新选择。";
      $(document.body).scrollTop(0);
      setTimeout(10, function() {
        vm.$router.push({ path: "/home" });
      });
    }
    var independentList = JSON.parse(independentCart);
    vm.independent = independentList[0];
  }
};
</script>
