<template>
    <div>
        <div v-if="independentShow || innovationShow">
            ...
            <div id="swiper" class="w clearfix detail_carousel">
                <div class="fl">
                    <div class="swiper-container gallery-top">
                        <div class="swiper-wrapper">
                            <div class="swiper-slide" v-for="image in images" :style="{backgroundImage:'url(' + image + ')'}"></div>
                        </div>
                    </div>
                ...
                <div class="fr">
                    ...
                    <p class="position" title="resAddress">{{resAddress}}</p>
                    <div class="acreage">办公面积：<span>{{parkAreaNum}}平米</span></div>
                    <div class="station">办公工位：<span>{{parkStationNum}}个</span></div>
                    <!-- <div style="font-size: 14px">{{resPosition}}</div> -->
                    <div class="order_visit"><a href="javascript:;" class="clearfix" @click="showVisit"><div class="img"></div><span>预约参观</span></a></div>
            ...
            <div class="w" v-cloak>
                    ...
                    <div class="innovation" v-if="innovation != null" v-show="innovationShow">
                        ...
                        <div class="description">
                            <div class="description_title">
                                <div class="small_vertical_bar"></div>
                                <p>园区描述</p>
                                <div class="decorate_ball"></div>
                                <div class="decorate_line"></div>
                            </div>
                            <div class="content">
                                <img v-bind:src="resDesImg" alt=""/>
                            </div>
                        </div>
                    </div>
                ...
                <div id="position" class="position" v-if="c == '2'">
                    <div class="description_title" v-show="independentShow || innovationShow">
                        <div class="small_vertical_bar"></div>
                        <p>地理位置</p>
                        <div class="decorate_ball"></div>
                        <div class="decorate_line"></div>
                    </div>
                    <div id="map_list" class="map_list" v-cloak>
                      <baidu-map v-if="isLoaded" class="bm-view" :center="transPoint" :zoom="mapInfo.zoom" @ready="handleMapReady">
                        <bm-navigation anchor="BMAP_ANCHOR_TOP_RIGHT"></bm-navigation>
                        <bm-marker :position="transPoint" :dragging="true" @click="infoWindowOpen">
                          <bm-info-window :show="mapInfoShow" @close="infoWindowClose" @open="infoWindowOpen">{{resName}}</bm-info-window>
                        </bm-marker>
                      </baidu-map>
                    </div>
                </div>
        ...
        <div v-show="showVisitMask" class="mask" v-cloak>
            <div class="visit_order_box">
                <div class="close"><img src="../assets/images/visit_mask_close.png" alt="" @click="hideVisit"/></div>
                <div class="title">{{resName}}</div>
                <p class="fail" v-show="error" v-cloak>{{message}}</p>
                <div class="second_title">请填写参观信息</div>
                <div class="date common_div">
                    <label for="">日期</label>
                    <Datepicker v-model="visitDate" class="startTime" language="zh" :disabled="state.disabled" :format="cusromFormatter"></Datepicker>
                </div>
                <div class="name common_div">
                    <label for="">姓名</label>
                    <input type="text" v-model="name"/>
                </div>
                <div class="contact common_div">
                    <label for="">联系电话</label>
                    <input type="text" v-model="phone"/>
                </div>
                <div class="btn"><a href="javascript:;" @click="submitVisitInfo">提交</a></div>
            </div>
        </div>
        <applyVisitSuccess></applyVisitSuccess>
    </div>
</template>
...
import applyVisitSuccess from "../component/applyVisitSuccess.vue";
...
export default {
  ...
  data() {
    return {
      ...
      mapInfo: {
        zoom: 8,
        point: null,
        mapCenter: { lng: 115.0014566098, lat: 30.1613536674 }
      },
      mapInfoShow: false,
      state: {
        disabled: {
  ...
  methods: {
    cusromFormatter(date) {
      this.visitDate = moment(date).format("YYYY-MM-DD");
    ...
    // 预约参观弹窗的显示与隐藏
    showVisit() {
      this.showVisitMask = true;
    },
    hideVisit() {
      this.showVisitMask = false;
      this.visitDate = null;
      this.name = null;
      this.phone = null;
      this.resId = null;
    ...
    handleMapReady() {
      var vm = this;
      var pArray = vm.resPosition.split(",");
      vm.transPoint = {
        lng: parseFloat(pArray[0]),
        lat: parseFloat(pArray[1])
      };
    },
    infoWindowClose() {
