        ...
        <div class="w clearfix">
            <div class="myOrder_tab_body detail_content" v-cloak>
                ...
                <div class="product_inventory independent_product_inventory">
                    ...
                    <table class="detail_table" v-if="screenWidth > 767">
                        <thead>
                        <th>产品类型</th>
                        <th>产品</th>
                        <th>可售面积</th>
                        ...
                        <tr>
                            <td>独立办公空间</td>
                            <td>{{independent.productName}}</td>
                            <td>{{independent.productArea}}平米</td>
                            <td>{{independent.productUnitPrice}}元/平米</td>
                            <td>{{independent.productQuantity}}平米</td>
                        </tr>
                        </tbody>
                    </table>
                    <div v-if="orderStatus == 99" class="orderSure_total_price" style="color: #666; font-size: 14px;">总价：<span class="red" style="font-size: 30px;">{{independent.totalPrice}}</span><span class="red" style="font-size: 14px">元</span></div>
                    <div v-else class="orderSure_total_price" style="color: #666; font-size: 14px; margin-bottom: 0.4rem;">总价：<span class="red" style="font-size: 30px;">{{independent.totalPrice}}</span><span class="red" style="font-size: 14px">元</span></div>
                    <div class="immediately_buy clearfix"><router-link class="pay" target="_blank" v-if="orderStatus == 99" :to="{path: '/payWayThird', query: {orderId: orderId, productDesc: productName, orderSn: orderSn, totalPrice: orderPrice, orderType: '3'}} ">支付</router-link></div>
                </div>
            </div>
        </div>
    </div>
      ...
      axios
        ...
        .then(function(response) {
          ...
          Object.assign(vm, {
            ...
            independent: resData.data.goodsItems[0],
            ...
            productName: resData.data.goodsItems[0].productName,
